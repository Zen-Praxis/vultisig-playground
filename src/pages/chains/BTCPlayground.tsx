...
import { getMethodComponent } from '../../components/bitcoin/methodMapping'
...
function BTCPlayground() {
  const [error, setError] = useState<string | null>(null)
  const [methodResults, setMethodResults] = useState<MethodResult>({})
  const providerMethods = useProviderMethods('bitcoin')

  useEffect(() => {
    if (window.vultisig?.bitcoin) {
      setError(null)
    } else if (window.vultisig) {
      setError('Vultisig extension detected, but Bitcoin provider is not available.')
    } else {
            ...
            {providerMethods.length > 0 && (
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-gray-900 mb-4">
                  Available Methods for BTC Provider
                </h3>
                  ...
                  {providerMethods.map((method) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {MethodComponent ? (
                          <MethodComponent
                            provider={window.vultisig?.bitcoin}
