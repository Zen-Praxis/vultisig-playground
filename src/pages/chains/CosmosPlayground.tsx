...
import { getMethodComponent } from '../../components/cosmos/methodMapping'
...
function CosmosPlayground() {
  ...
  useEffect(() => {
    ...
    if (hasAnyProvider) {
      ...
      setError('Vultisig extension detected, but Cosmos providers are not available.')
    } else {
  ...
  const createHandlers = useCallback((providerName: string, methodName: string) => {
    return {
  ...
  const getProviderInstance = (providerName: string): unknown => {
    return (window.vultisig as Record<string, unknown>)[providerName]
  }
              ...
              {provider.methods.length > 0 && (
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h3 className="text-sm font-semibold text-gray-900 mb-4">
                    Available Methods for {provider.name.charAt(0).toUpperCase() + provider.name.slice(1)} Provider
                  </h3>
                  <div className="space-y-3">
                    {provider.methods.map((method) => {
                      const methodResult = methodResults[provider.name]?.[method]
                      const MethodComponent = getMethodComponent(method)
                      const { handleResult, handleError } = createHandlers(provider.name, method)
                      ...
                      return (
                        ...
                        >
                          ...
                          {MethodComponent ? (
                            <MethodComponent
                              provider={getProviderInstance(provider.name)}
